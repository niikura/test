...
%TBD
%\begin{comment}
By fully taking advantage of the wide field-of-view of Hyper Suprime-Cam, 
which allows us to cover the entire bulge and disk regions of Andromeda Galaxy (M31) 
...
matter in both the halo regions of Milky Way and M31. 
We aim at constraining the event rate of microlensing from PBH, with the 
dense-cadence HSC data. 
However, the PBH search requires an exquisite data 
...
HSC pipeline to make the data reduction and remove background events with 
statistical cuts, with event selections. 
We constrained the abundance of PBHs of mass scales, 
$10^{-9}$-$10^{-7}M_\odot$ and derived the strongest limits of 
PBH in the dark matter halo from the null detection of microlensing event. 
%the current status of this project (the results of other transients and the PBH microlensing search). 
We find that...
%\end{comment} 
\end{abstract}
...
timescale of microlensing event  $t_0={R_E}/{v}$ %as in Eq.~(\ref{eq:mtimescale}) 
is given by: 
%
\begin{eqnarray}
...
\end{eqnarray}
%
where $A_0$ represents the maximum magnification of the light curve. Here we will characterize the timescale as a function of impact parameter $\beta$, the quantity described as $u_\mathrm{min}$ in Eq.~(\ref{eq:cimpa}). Then we can rewrite the timescale as: %in the same way as described in \citet{Gondolo:99}: 
%
\begin{eqnarray}
...
%The visibility (elevation) of M31 and the seeing size of each exposure are given in Fig.~\ref{fig:seeing}. Here the ``seeing'' is a commonly-used quantity to characterize a spatial resolution of an image, i.e. the size of the point spread function (PSF) of the image. 
...
%Fig.~\ref{fig:seeing} shows how the seeing size changes with time from the observation start. 
...
%Almost all the stars are cleanly subtracted. 
...
background subtraction in difference images %median subtraction 
...
Fig.~\ref{fig:m31fit} shows %compares 
the distributions of identified candidates based on %the different subtraction methods, the spline-fitting method (right panel: the default method of HSC pipeline) and 
the polynomial fitting method developed in this paper. %(left panel). 
...
%The right panel shows the distribution of variable candidates obtained when using the spline fitting method for background subtraction in each CCD chips (the default method of HSC pipeline). For this case we found about $1500$ candidates in total. 
The different color symbols are different types of variable candidates classified based on their light curves. 
%Left panels displays the distribution of variable candidates obtained by using the 6 or 10-th order polynomial fitting method for each CCD chip (see text for details). 
...
and choose 11,462 candidates by the number of detection. %, detected more than twice
...
%(In Kepler analysis they divide good fluxes by mean flux of the lightcurve which is calculated over 300 data points towards the center of the lightcurve, and subtract unity from this quantity to produce a fractional magnification lightcurve.)
...
(autocorrelationの条件は入れた方がよいのか?--->  短時間なので今日は大丈夫そう)

まずは変光天体の候補を抽出する
3枚足した画像、差分画像を作成し、2回以上検出されたイベントを変光イベントのcandidateとする。
ここからマイクロレンズの候補イベントを抽出する。


#thresholdを超えるピークを含むイベントのみ選ぶ
まずはじめに、十分大きなピークを含むイベントを選ぶため、
ノイズのthresholdを設けた。
thresholdには、シミュレーションのノイズを選んだ。
このthresholdを3点以上連続で超えたライトカーブを
ピークを含むイベントと見なした。
殆どのイベントはこのthresholdを超えていた。
(efficiencyの計算の時も同様の条件を考慮している。)


#次にマイクロレンズでフィッティングする。
各イベントに対してマイクロレンズのライトカーブでフィッティングする。
今回は対象としているPBHの質量が小さいので、補正なしの式を用いる。
イベントレート的に1回程度なので、7時間の観測のライトカーブで一番大きなピークの時刻をピーク時刻とする。
microlensingのパラメータは4つで、t0 (ピークの時刻-sourcelensが最も近い時)、^t (the duration of the event)、Amax (the peak magnification)、rho_min (the distance of closest approach between lens and source lines-of-sights divided by the stellar radius in the lens plane)
0<beta<2、timescale 0<t<25000secでパラメータがでてこなかったものはreject. 
パラメータはでてくるが、カイ二乗が悪すぎるものがるので、それを切り捨てる。
かなりの数が切り捨てられる。

ここまでで、ピークがあるイベントがのこる。まずはfakeをとりのぞく。

#CCD edgeによる増光
まずfakeなイベントを取り除く
増光ピークを持つイベントのうち、CCDedge付近は測光がうまくいかない。
CCDの座標を元に、端から5pixel以内に近づく事があるイベントをbad eventsとしてとりのぞく。
いくらか切り捨てられる。

#spikeによる増光。
測光できないbad eventsやfake eventsをreject。
多くはspikeで、近傍の星からのoutliers?が重なることによる増光が原因。
差分画像上で41pixel四方から、中心付近5pixelのカウント値を引き算して、
中央以外のカウント値が平均分散よりも大きい場合はrejectする。

#その他のdefect (さいご?)
他にも、CCD defectや画像のstripeがfakeイベントとして検出される。
これらはカウントを等級に直すと突発的に現れるケース。取り除いてあげる。
#小惑星もあったけれども、同様にとりのぞいてしまいました。
移動の軌跡が見えてしまうことを利用出来ないか? (bumpのvariabilityのlocal measurement..?CCD edgeの軌跡と酷似しているもの。)

ここまでで最も大きなbumpのcategory(数百イベント): 


ここまできて残ったイベントは(#あとはリアルっぽいイベントなので、分類。)

#ちょっとした変光星との分類は
多くはshort term variabilitydaga,
周期長いイベントは端が切れてしまっている、ピークを持たないイベントはsimulationでも適用外であり、今回は考慮しない事にする。
chi2in chi2outつかうとある程度除ける。 

#seeingの相関。(いいのかな??)
残ったイベントの傾向を見てみると、fittingが良いピークのうち、なだらかなものは多くは同じような時刻にピークを持つ。
これはseeingのよい時刻と相関がありそう。相互相関をとって、強すぎるものはfakeとして取り除いた。


#フレア非対称性。(フレアフィッティング。)
かなり非対称なbump -> stellar atmosphere でのmagnetic activityによるstellar flareが考えられる。
解決策: 4-5点の連続した点+1.5*flare timescale lambda or 2*^t でのasymmetry で、asymmetryが17%より小さいこと(+上のchi^2の条件から、short-durationも取り除ける)
今回はサンプリングが十分と考えられるので、short duration eventについても取り除くことができた。
(exponentialでfittingもしたが、short durationのイベントは特にうまくゆかなかったため、非対称性の条件だけで取り除くことにした)
(flareのパラメータはt0 (ピークの時刻-sourcelensが最も近い時)、Amax (the peak amplitude)、lambda(exponential decay constant)
立ち上がりは30分のうちどこかで起こると仮定。測定値が瞬時にpeakの測定値を超えるときは、平均peak fluxはbaselineとpeakの値の間の値を持つ。
なので、flareにはもう1つパラメータ(peak timeをすぎてすぐのfluxの値)を加える。
#flareをとりのぞく条件: mlchi2dof <  0. 75Å~fchi2dof )
...
\begin{itemize}
%
\item{Noise threshold}\\
Our requirement of microlensing event is to contain 
a significant bump in the light curve. 
Therefore we impose a noise threshold on the lightcurves 
so as to select events with significant bumps. 
The threshold is derived from the variance of flux 
of difference images, measured at 1,000 random points. 
We require the secure events to have large enough bumps 
with more than 3-consecutive points 
above the absolute value of noise cut 
in their lightcurves. 



\item{Fitting of microlensing shape to lightcurves}\\
As expected from the microlensing event rate described in \S~\ref{sec:constraint}, 
the number of microlensing bump expected in our 7-hour long lightcurve would be one at most. 
...
%Note that this process took a very long time, 
...
\item{CCD defects}\\%last condition?
...
from the unique path of defects similar to the movement of CCD edge during our observation. 
...
\item{Variable stars}\\%rejection by peak, short term variability data
...
After removing the events no PBH microlensing candidates exists. 
...
% \hline 
   \label{table:counta}
	...
	%\begin{tablenotes}
	%   \item {
         \tablecomments{\small ($*$) is only applied to HSC data }
	%\end{tablenotes}
	   \end{center}
   \end{table*}
%
...
\begin{comment}
まずハローモデル。
M31とMWのハローモデルとして、別のモデルを使ってみた場合の不定性。
\end{comment}
...
The best-fit relations derived from the best-fit linear or parabola are as follows: %with huge error bars
%
\begin{eqnarray}
...
%Thus we adopted HST-corrected number density count for the field by extending up to 26 magnitude. 
